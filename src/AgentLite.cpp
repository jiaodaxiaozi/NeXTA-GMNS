...
void ReadLinkTollScenarioFile(Assignment& g_assignment)
{
	...
	// generate toll based on demand type code inroad_ink.csv file
...
void g_ReadInputData(Assignment& g_assignment)
{
	...
	if (parser.OpenCSVFile(g_AgentLiteProjectDirectory + "node.csv", true))
	{
	...
	if (parser_link.OpenCSVFile(g_AgentLiteProjectDirectory+"road_link.csv", true))
	{
...
void g_ReadNewDemandFile(Assignment& g_assignment)
{
	...
	if (parser.OpenCSVFile(g_AgentLiteProjectDirectory+"input_new_demand_file_list.csv", true))
	{
		...
		while (parser.ReadRecord())
		{
			...
			{
				cout << "Error: format_type = " << format_type << " is not supported. Currently DTALite supports multi_column, matrix, full_matrix, agent_csv, agent_bin, trip_csv,transims_trip_file." << endl;
				g_ProgramStop_agentlite();
