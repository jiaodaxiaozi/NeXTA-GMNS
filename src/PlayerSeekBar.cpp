...
CString g_time_to_string(float timestamp)
{
   ...
   int second = (int)((timestamp - hour*60 - min)*60);
   float FSecond = ((timestamp - hour * 60 - min) * 60);

   int decimal_second = (int)(fmod(FSecond * 10, 10)+0.5);


	   str.Format("day [%d] %02d:%02d:%02d.%01d", day,hour, min,second, decimal_second);
